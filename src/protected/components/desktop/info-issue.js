    ...
    constructor() {
        ...
        this.$shadowRoot.append(/*html*/`
            <div class="modal-container">
                <div class="modal">
                    <h3 class="title">Information Issue</h3>

                    <div class="option hover">
                        <input type="checkbox">
                        <h4>This is not an Off Market Deal</h4>
                    </div>

                    <div class="option hover">
                        <input type="checkbox">
                        <h4>This is a ${$('deal-view')[0].deal.category === "SFH Deal" ? 'Land' : 'SFH'} Deal</h4>
                    </div>

                    <div class="option hover">
                        <input type="checkbox">
                        <h4>Incorrect Street Name</h4>
                    </div>

                    <div class="option hover">
                        <input type="checkbox">
                        <h4>Incorrect Street Number</h4>
                    </div>

                    <div class="option hover">
                        <input type="checkbox">
                        <h4>Incorrect City</h4>
                    </div>

                    <div class="option hover">
                        <input type="checkbox">
                        <h4>Incorrect State</h4>
                    </div>
        ...
        this.$shadowRoot.find('.submit-button').on('click', async event => {
            const selectedIssues = $(event.currentTarget).closest('.modal').find('input[type="checkbox"]:checked').map((_, element) => {
                if ($(element).parent().find('.other-input')[0]) {
                    ...
                    return $(element).parent().find('h4').text()
                }
            }).get() 
